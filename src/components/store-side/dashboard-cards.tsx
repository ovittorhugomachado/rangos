                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between gap-2 p-2">
                                ...
                                />
                                <h1 className={`${fontSize === 'text-sm' ? 'text-lg' : 'text-xl'} flex-2`}>
                                    {card.name}
                                </h1>
                                <span className="w-7 h-7 rounded-full flex items-center justify-center bg-white dark:bg-black text-black dark:text-white text-sm">
                                    {cardOrders.length}
                                </span>
                            </div>
                            {cardOrders.length === 0 ? (
                                <p className={`${fontSize} text-center py-4 ${activePanel.includes(card.id) ? 'block' : 'hidden'}`}>
                                    Nenhum pedido
                                </p>
                            ) : (
                                <div className={`${fontSize} ${card.textColor} flex flex-col items-center px-4 ${activePanel.includes(card.id) ? 'block' : 'hidden'}`}>
